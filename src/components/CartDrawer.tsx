    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle className="font-mono uppercase tracking-widest">Carrinho</SheetTitle>
          <SheetDescription>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 border border-border rounded-sm">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-semibold text-sm mt-1">R$ {parseFloat(item.price.amount).toFixed(2)}</p>
                      </div>
