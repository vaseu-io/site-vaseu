    ...
    for (const product of data.data) {
        ...
        if (product.skus && product.skus.data) {
            for (const sku of product.skus.data) {
                ...
                console.log(`  SKU: ${sku.sku} | Token: ${sku.token} | Variations: [${variations}]`);
            }
        }
    }
}
...
async function main() {
    await searchProduct('Oversized Basic Black');
    await searchProduct('Oversized Basic White');
    await searchProduct('Boxy Basic Black');
    await searchProduct('Boxy Basic White');
}
